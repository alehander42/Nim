...
However, certain constructs invalidate the value `not-nil`-ness. Calls to functions, and assignments where the compiler can't prove 
variables aliased to such value are not changed, lead to the compiler treating it as nilable again
