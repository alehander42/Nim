import os, strformat, strutils, sequtils, tables, sets, ropes, json, algorithm
...
proc parse*(source: string, path: string, mangled: HashSet[string]): SourceNode =
  ...
  for i, originalLine in lines:
    ...
    if line.startsWith("// line"):
      ...
      let linePath = line[pos + 1 .. ^1] # quotes
      ...
      for j, token in tokens:
        ...
        if result.children.len > 0:
          ...
          result.children.add(
            child(
              newSourceNode(i + 1, 0, path, token[1] & nl, name)))
...
proc toStringWithSourceMap*(node: SourceNode, file: string): tuple[code: string, map: SourceMapGenerator] =
  var code = ""
...
proc genSourceMap*(source: string, mangled: HashSet[string], path: string, outfile: string): (Rope, SourceMap) =
  let node = parse(source, outfile, mangled)
  let map = node.toStringWithSourceMap(file = outfile)
